...
<script setup lang="ts">
  ...
  import { categories, components, statusClassMap, statusTextMap } from './data'

  const currentCategory = ref('')

  const filteredComponents = computed(() => {
    if (!currentCategory.value) return components
    return components.filter(comp => comp.category === currentCategory.value)
  })

  const getComponentCountByCategory = (categoryKey: string) =>
    components.filter(comp => comp.category === categoryKey).length

  const getCategoryName = (categoryKey: string) =>
    categories.find(cat => cat.key === categoryKey)?.name ?? '未知分类'
