...
<template>
  ...
  >
    <view class="profile-page">
      ...
      <view class="user-card">
        ...
        <view class="card-content">
          ...
          <view class="edit-btn" @click="goToSettings">
            <wd-icon name="edit-outline" size="16px" color="#667eea" />
          </view>
        </view>
      </view>
      ...
      <!-- 功能菜单 -->
      <view class="menu-section" v-for="group in menuGroups" :key="group.title">
          ...
          >
            <view class="menu-left">
              <view class="menu-icon-wrap" :style="{ background: item.iconBg }">
                <wd-icon :name="item.icon" size="18px" color="#fff" />
              </view>
...
const menuGroups = ref([
  ...
  },
  {
    title: '开发工具',
    items: [
      { id: 'components', label: '组件库', icon: 'app', iconBg: 'linear-gradient(135deg, #4facfe, #00f2fe)', extra: '20个', path: '/pages/demo/index' },
      { id: 'docs', label: '开发文档', icon: 'books', iconBg: 'linear-gradient(135deg, #f093fb, #f5576c)' },
      { id: 'changelog', label: '更新日志', icon: 'calendar', iconBg: 'linear-gradient(135deg, #a8edea, #fed6e3)', extra: 'v1.0.0' },
    ]
  },
  {
    title: '其他',
...
const handleMenuClick = (item) => {
  ...
  const actions = {
    ...
    changelog: () => uni.showModal({ title: '更新日志', content: 'v1.0.0\n- 迁移 wot-design-uni 组件库\n- 搭建基础设施\n- 封装 15 个 P0 通用组件\n- 新增消息中心、个人设置、主题切换', showCancel: false }),
    feedback: () => uni.showToast({ title: '反馈功能开发中', icon: 'none' }),
    about: () => uni.showModal({ title: 'Robot UniApp', content: '企业级跨平台应用开发框架\n\nVue 3.5 + UniApp + wot-design-uni\n\n© 2025 CHENY', showCancel: false }),
    cache: () => {
      uni.showModal({
        title: '清除缓存',
        content: '确定要清除应用缓存吗？',
