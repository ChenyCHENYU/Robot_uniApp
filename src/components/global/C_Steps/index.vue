<template>
  <view
    ...
    :style="{ '--steps-active-color': activeColor, '--steps-inactive-color': inactiveColor }"
  >
    <view v-for="(step, index) in steps" :key="index" class="c-steps__item">
      <!-- 连接线 -->
      <view
      ...
      <!-- 指示器 -->
      <view :class="['c-steps__indicator', `c-steps__indicator--${getStatus(index)}`]">
        <wd-icon v-if="index < current" name="check" size="14px" color="#fff" />
        <text v-else>{{ index + 1 }}</text>
      </view>

      <!-- 内容 -->
      <view class="c-steps__content">
        <view :class="['c-steps__title', index > current && 'c-steps__title--inactive']">
          {{ step.title }}
        </view>
        <view v-if="step.description" class="c-steps__desc">{{ step.description }}</view>
      </view>
    </view>
  </view>
</template>
