<template>
  <view class="c-upload">
    ...
    >
      <image class="c-upload__preview" :src="file.url || file" mode="aspectFill" />

      <!-- 上传进度 -->
      <view v-if="file.status === 'uploading'" class="c-upload__progress">
        <view class="c-upload__progress-bar" :style="{ width: (file.progress || 0) + '%' }" />
      </view>

      <!-- 上传失败 -->
      <view v-if="file.status === 'error'" class="c-upload__status">
        <wd-icon name="warning" size="24px" color="#fff" />
      </view>

      <!-- 删除按钮 -->
      <view v-if="deletable && !disabled" class="c-upload__delete" @click.stop="onDelete(index)">
    ...
    <view
      ...
      @click="onChoose"
    >
      <wd-icon name="add" size="32px" color="#c0c4cc" />
      <text class="c-upload__add-text">{{ modelValue.length }}/{{ maxCount }}</text>
    </view>
  </view>
</template>
...
const emit = defineEmits(['update:modelValue', 'choose', 'delete', 'preview', 'oversize'])
