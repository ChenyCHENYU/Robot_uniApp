<template>
  <view v-if="visible">
    <view class="c-number-keyboard__overlay" @click="close" />
    <view class="c-number-keyboard__panel">
      <!-- 标题栏 -->
      <view v-if="title" class="c-number-keyboard__header">
        <text class="c-number-keyboard__title">{{ title }}</text>
        <text class="c-number-keyboard__done" @click="onDone">完成</text>
      </view>
      ...
      <view class="c-number-keyboard__grid">
        ...
        >
          <wd-icon v-if="key.type === 'delete'" name="arrow-left" size="22px" />
...
import { defaultProps, generateKeys } from './data.js'
...
const emit = defineEmits(['update:visible', 'update:modelValue', 'input', 'delete', 'done'])

const keys = computed(() => generateKeys(props.showDot, props.extraKey, props.randomOrder))
...
const onKeyPress = (key) => {
  ...
  if (props.maxLength > 0 && props.modelValue.length >= props.maxLength) return
