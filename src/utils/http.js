...
class Http {
   ...
   */
  addPageTask(pageRoute, requestTask) {
   ...
   */
  removePageTask(pageRoute, requestTask) {
    const tasks = this.pageTasksMap.get(pageRoute)
    if (tasks) {
   ...
   * @private
   */
  async _executeWithRetry(reqConfig, retryCount, retryDelay) {
   ...
   * @private
   */
  async _doRequest(reqConfig, retriesLeft, retryDelay) {
    try {
   ...
   * @private
   */
  async _handleError(error, silent) {
    // 401 → 保存当前路由 → 跳转登录
    if (error.code === 401) {
      const userStore = useUserStore()
